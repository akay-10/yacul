...
// LockRange: A thread-safe, production-grade range locking facility.
//
...
//   - Upgrade path: shared -> exclusive on a held guard.
//
...
public:
  ...
  enum class Status : uint8_t {
    ...
    kCancelled = 2, // Acquisition cancelled (reserved for future use).
  };
  ...
  public:
    ...
    // Upgrade a kShared lock to kExclusive.  Blocks indefinitely.
    // Throws std::logic_error if the guard is not valid or already exclusive.
    // After a failed upgrade (exception) the guard is invalidated; the
    // caller no longer holds any lock on the range.
    void Upgrade();

    // Timed upgrade variant.  Returns kAcquired on success, kTimeout on
    ...
    // Accessors (only meaningful when Valid() == true).
  ...
  LockRange(const LockRange &) = delete;
  LockRange &operator=(const LockRange &) = delete;
  LockRange(LockRange &&) = delete;
  LockRange &operator=(LockRange &&) = delete;
  ...
  // Purely informational; no happens-before guarantee.
...
private:
  ...
  struct LockEntry {
    uint64_t offset = 0;
    uint64_t end = 0; // exclusive end: offset + length
    Mode mode = Mode::kShared;
    uint32_t share_count = 1; // number of concurrent shared holders
  };

  // A pending waiter descriptor.  Stack-allocated by the waiting thread;
  // a pointer is stored in waiters_.
  struct Waiter {
    uint64_t offset;
    uint64_t end;
    Mode mode;
  ...
  struct EntryKey {
    uint64_t offset;
    uint64_t id; // monotonically increasing, unique per entry
  ...
  // True if the range can be granted immediately (no conflict, no writer
  // priority block).
  ...
  // Core wait loop.  Caller must hold `lk` (which wraps mu_) on entry;
  // lk is still held on return.  `exclude_key` is forwarded to CanGrant
  // to allow the upgrade path to ignore the caller's own existing entry.
  ...
  std::condition_variable cv_; // broadcast on every lock release / change
