...
// ---------------------------------------------------------------------------
// TaskHandle
// Returned when submitting delayed / recurring tasks.
// The caller can use it to cancel the task.
// ---------------------------------------------------------------------------
class TaskHandle {
public:
...
struct ThreadPoolStats {
  ...
  double GetCompletionRate() const {
    ...
    return (submitted > 0) ? static_cast<double>(tasks_completed.load(
...
struct DelayedTask {
  ...
  DelayedTask(std::chrono::steady_clock::time_point exec_time,
      ...
      : execute_time(exec_time), priority(prio), task(std::move(t)),
        task_id(id) {}

  // std::priority_queue is a max-heap (largest element on top).
  // We want earliest time to execute first, so invert comparison.
...
public:
  ...
  auto SubmitWithPriority(TaskPriority priority, F &&f, Args &&...args)
    ...
    EnqueueTask(priority, [this, shared_task]() mutable {
  ...
  TaskHandle SubmitRecurringWithPriority(std::chrono::milliseconds interval,
    ...
    *body = [this, interval, priority, cancelled, task_id, user_fn,
      ...
      try {
        ...
        stats_.tasks_failed.fetch_add(1, std::memory_order_relaxed);
        LOG(ERROR) << "Recurring task " << task_id << " threw an exception.";
      }
...
private:
  ...
  struct PriorityTask {
    ...
    // std::priority_queue is a max-heap (largest element on top).
    // Higher priority enum value = higher priority = should execute first.
  ...
  // Enqueue a pre-wrapped task onto the appropriate regular queue.
  // Must be called WITHOUT queue_mutex_ held.
  ...
  // Attempt to steal one task from a peer thread-local queue.
  // Returns nullptr if nothing is available.
  std::function<void()> StealTask(uint32_t thief_id);

  // Dequeue the next task from the shared priority / FIFO queue.
  // Returns false if no task is available.  Must be called with queue_mutex_.
  ...
  // Sized to max_threads at construction; index == thread_id % max_threads.
...
// ---------------------------------------------------------------------------
// GlobalThreadPool – singleton wrapper
// ---------------------------------------------------------------------------
