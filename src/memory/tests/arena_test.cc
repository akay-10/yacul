...
TEST(ArenaTest, Growth) {
  ...
  for (int k = 0; k < N; ++k) EXPECT_EQ(*ptrs[k], k);

  cout << "Allocated " << N << " ints across "
       << arena.block_count() << " block(s)\n";
  cout << LOGVARS(arena.reserved_bytes(), arena.total_allocated()) << '\n';
}

TEST(ArenaTest, Reset) {
  Arena arena(1024);
  for (int k = 0; k < 100; ++k) arena.Alloc<int>(k);
...
TEST(ArenaTest, MarkRewind) {
...

  {
    ...
    cout << "Inside mark: "
         << LOGVARS(allocated_inside, arena.block_count()) << '\n';
...
TEST(ArenaTest, ArrayAllocation) {
  ...
  for (size_t i = 0; i < N; ++i) buf[i] = static_cast<float>(i) * 0.5f;
...
TEST(ArenaTest, STLAdaptor) {
  ...
  for (int k = 0; k < 1000; ++k) v.push_back(k);
...
TEST(ArenaTest, TreeInArena) {
  ...
  auto Insert = [&](auto& self, Node*& root, int val) -> void {
    if (!root) { root = arena.Alloc<Node>(); root->value = val; return; }
    if (val < root->value) self(self, root->left, val);
  ...
  for (int v : values) Insert(Insert, root, v);
