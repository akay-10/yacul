...
public:
  ...
  // Callback invoked just before the child is (re)started. 'attempt' is 0-based
  // (0 = first start, 1 = first restart, …).
  ...
  struct Config {
    ...
    // Maximum number of consecutive restarts before 'max_restart_policy' is
    // applied. 0 means unlimited.
    int max_restarts = 0;
    ...
    // How often the monitor thread polls the health-check hook. Ignored when
  ...
  // Start monitoring. Spawns the monitor thread which immediately forks the
  // child. Returns true on success; false if already running or if 'cfg' is
  // invalid (e.g. empty argv).
  ...
  // Block until the monitor thread has fully exited. Safe to call even if the
  // monitor has already stopped.
  static void WaitForStop();

  // Send an arbitrary signal to the currently running child. Returns false if
  // no child is alive or signal delivery fails.
